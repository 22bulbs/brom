...
function transactionToData(data, transaction) {
  //grab method, external and flags from transaction's metadata
  let { method, external, flags } = transaction.metadata;

  //if the method of the transaction is already present in the methods object, increment its value, otherwise make it equal to 1
  ...
  //increment external or local property depending on the transaction's type
  if (external) {
