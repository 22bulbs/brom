  ...
  const checkForHalts = (results) => {
    ...
    results.forEach(result => {
      if (result instanceof Error) {
        console.error(result); // do we crash on errors? or is that within the realm of "normal" tests
      } else {
        if (result.halt) {
    ...
    if (halts.length) {
      console.error(`Brom tests failed. Rules failed on ${halts.length} routes.`);
      halts.forEach(route => {
