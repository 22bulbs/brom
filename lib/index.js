...
process.env.WEBHEAD_RESULTS_PORT = config['results-port'] || 7913;
process.env.WEBHEAD_PROXY_PORT = config['proxy-port'] || 9999;
...
if (options.https) {
  if (!config.https.key || !config.https.cert) {
    console.error('HTTPS mode requires relative paths to both .key and .crt, please include in webhead.config.json');
    process.exit(1);
  } else {
    process.env.WEBHEAD_USE_HTTPS = true;
    process.env.WEBHEAD_HTTPS_KEY = path.resolve(process.cwd(), config.https.key);
    process.env.WEBHEAD_HTTPS_CERT = path.resolve(process.cwd(), config.https.cert);
  }
}
