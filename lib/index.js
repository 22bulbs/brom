...
if (options.help) {
  ...
  OPTIONS:
    ...
    -r, --read-file=FILENAME      Open a results file saved by brom to review
                                  the session in the client               
...
if (options['disable-autostart'] && options.automated) {
  console.error('Automated analysis can not be used in conjunction with disable-autostart flag');
  process.exit(1);
}

if (!config.server && !options['disable-autostart'] && !options['read-file']) {
...
if (!config.port && !options['read-file']) {
...
process.env.BROM_TITLE = config.title || `localhost:${config.port}`;

if (options['read-file']) {
  const readFile = path.resolve(process.cwd(), options['read-file']);
  if (fs.existsSync(readFile)) {
    process.env.BROM_READ_DESTINATION = readFile
  } else {
